...
namespace IntranetSystem.QRCode
{
    ...
    public class QRCodeController : Controller
    {
        ...
        [ValidateInput(false)]
        public ActionResult doCreate(QRCODE param, String ary)
        {
          ...
          //  param.CONTENT =  JsonConvert.DeserializeObject<String>(param.CONTENT);
            ...
            var httpPostedFile2 = Request.Files["IMG2"];
            JObject jo = new JObject();
            try
            {
                FormsIdentity id = (FormsIdentity)User.Identity;
                ...
                if (Request != null && httpPostedFile != null)
                {
                    ...
                    //param.COUPON_IMG = Request.Files["COUPON_IMG"].FileName;
                }
                if (Request != null && httpPostedFile2 != null)
                {
                    uploadCoupon(Request.Files["IMG2"]);
                    param.COUPON_IMG = Request.Files["IMG2"].FileName;
                }


                if (param.ISSHOW == "on" || param.ISSHOW=="1")
                {
                    param.ISSHOW = "1";
                }
                else
                ...
                if (param.CUS_SAVE == "on" || param.CUS_SAVE == "1")
                {
                    param.CUS_SAVE = "1";
                }
                else
                ...
                int i = qRCodeService.doCreate(param,obj);
                jo.Add("status", i);
            }
            catch (Exception e)
            {
        ...
        public ActionResult doCreateDTL(QRCODE_DTL param)
        {
            JObject jo = new JObject();
            try
            {
                ...
                int i = qRCodeService.doCreateDTL(param);
                jo.Add("status", i);
            }
            catch (Exception e)
            {
        ...
        [ValidateInput(false)]
        public ActionResult doEdit(QRCODE param,String ary)
        {
            JArray obj = JsonConvert.DeserializeObject<JArray>(ary);
            //param.CONTENT = JsonConvert.DeserializeObject<String>(param.CONTENT);
            var httpPostedFile = Request.Files["IMG"];
            ...
            try
            {
                ...
                int i = qRCodeService.doEdit(param,obj);
                jo.Add("status", i);
            }
            catch (Exception e)
            {
        ...
        public ActionResult getDepartSelect()
        {
            JArray ja = new JArray();
            JObject jo = new JObject();
            ja = qRCodeService.getDepartSelect();
            return Content(JsonConvert.SerializeObject(ja), "application/json");
        }
