...
namespace HowardCoupon_vs2010.Models.Service
{
    public class PayTypeService
    {
        ...
        public JObject getPayType(EasyuiParamPoco param,String CODE)
        {
            JObject jo = new JObject();
            JArray ja = new JArray();         
            var query = from t in db.IMC_CODE where t.KIND=="COUPON" && t.TYPE=="PAY" select t;
        ...
        {
            var query = from t in db.IMC_CODE where t.KIND=="COUPON" && t.TYPE=="PAY" && t.CODE == CODE select t;
            return query.Count();
        }
