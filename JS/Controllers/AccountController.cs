...
namespace Jasper.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            Hashtable ht = employeeService.getEmployee(model.UserName, model.Password);
            if (ht.Count == 0)
            {
            ...
            {
                Session.RemoveAll();
                String userid = HashtableUtil.getValue(ht, "USERID");
                String roleid = HashtableUtil.getValue(ht, "ROLEID");
                LoginProcess(userid,roleid, model.RememberMe);
                return RedirectToAction("frm201", "frm201");
            } 
        }

        private void LoginProcess(String userid,String roleid, bool isRemeber)
        {
            var now = DateTime.Now;
           // String roles = employeeService.getRoleAryStr(userid);
        ...
        {
            //WebSecurity.Logout();
            var identity=System.Web.HttpContext.Current.User.Identity as FormsIdentity;

            if (User.Identity.IsAuthenticated)
            {
                /*
                String cacheKey = String.Concat("UserRoles_", identity.Ticket.UserData);
