//------------------------------------------------------------------------------
// <auto-generated>
//    這個程式碼是由範本產生。
//
//    對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//    如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace Jasper.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class view_MKRPAY
    {
        public string MKRNO { get; set; }
        public string MKRNM { get; set; }
        public int FEEAMT { get; set; }
        public string VNO { get; set; }
        public string FEEYM { get; set; }
        public string NOTES { get; set; }
        public short PAYSTA { get; set; }
        public Nullable<System.DateTime> LOGDT { get; set; }
        public string LOGUSR { get; set; }
        public string TAGID { get; set; }
        public string RSTA { get; set; }
        public Nullable<System.DateTime> ENDDT { get; set; }
        public string ACCNO { get; set; }
        public string ACCTP { get; set; }
        public string ACCNM { get; set; }
        public Nullable<int> AMTTX { get; set; }
        public Nullable<int> AMTSUM { get; set; }
        public Nullable<System.DateTime> VDT { get; set; }
        public Nullable<byte> FIXAREA { get; set; }
        public Nullable<byte> FIXQUIP { get; set; }
        public string KPISUB { get; set; }
        public string INBUDG { get; set; }
        public Nullable<System.DateTime> RPTDT { get; set; }
        public string REQUEST_NO { get; set; }
        public string TPNM { get; set; }
        public string CNM { get; set; }
        public Nullable<int> BUDGET { get; set; }
        public Nullable<int> USEAMT { get; set; }
        public string YRN { get; set; }
    }
}
