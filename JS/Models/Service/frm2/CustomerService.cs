    ...
    public class CustomerService 
{
    ...
    public JObject getCusTab1DG(String TAGID)
   {
       JArray ja = new JArray();
       JObject jo = new JObject();
       var query = from t in db.RMLIV where t.JOBTAG==TAGID && t.RSTA=="" orderby t.CIDT ascending select t;
    ...
    //取得住戶combobox
    public JArray getOWNER(String TAGID)
    {
        JObject jo = new JObject();
        JArray ja = new JArray();
        ...
        var query = (from t in db.RMLIV where t.JOBTAG == TAGID && t.RSTA == "" select new { CNO = t.MAN, CNM = "住戶" }).Distinct().Union
                    (from t in db.RMCAR where t.JOBTAG == TAGID && t.RSTA == "" select  new { CNO = t.DRIVER, CNM = "司機" }).Distinct();
    ...
    //取得住戶combobox
    public JArray getOWNER2(String TAGID)
    {
        JObject jo = new JObject();
        JArray ja = new JArray();
        ...
        var query = (from t in db.RMLIV where t.JOBTAG == TAGID && t.RSTA == "" select new { CNO = t.MAN }).Distinct();
    ...
    //取得住戶combobox
    public JArray getOWNER2MORE(String TAGID,String CNO)
    {
        JObject jo = new JObject();
        JArray ja = new JArray();
        ...
        var query1 = (from t in db.CONTRAH where t.RMNO == CNO && t.CNSTA == "C" select t).OrderByDescending(t => t.ENDYM).FirstOrDefault();

        if (query1 != null)
        {
            var query2 = (from t in db.RMLIV where t.JOBTAG == query1.TAGID && t.RSTA == "" select new { CNO = t.MAN }).Distinct();
            foreach (var item in query2)
            {
                var itemObject3 = new JObject
                    {   
                        {"CNO","(前租戶)"+ item.CNO },
                        {"CNM","(前租戶)" + item.CNO}
                    };
    ...
    public JObject getCusTab2DG(String TAGID)
    {
        JArray ja = new JArray();
        JObject jo = new JObject();
        var query = from t in db.RMTEL where t.JOBTAG == TAGID && t.RSTA == "" orderby t.TAGID ascending select t;
        foreach (var item in query)
        {
    ...
    public JObject getCusTab3DG(String TAGID)
    {
        JArray ja = new JArray();
        JObject jo = new JObject();
        var query = from t in db.RMCAR where t.JOBTAG == TAGID && t.RSTA == "" orderby t.TAGID ascending select t;
        foreach (var item in query)
        {
            var itemObject = new JObject
                    {                                           
    ...
    public JArray getPARKNO(String TAGID)
    {
        JObject jo = new JObject();
        JArray ja = new JArray();
        var query = (from t in db.CONTRAP where t.JOBTAG == TAGID && t.CNSTA == "" select new { CNO = t.PARKNO , CNM = t.LOCT });
        foreach (var item in query)
        {
            var itemObject = new JObject
                    {   
    ...
    public JObject getCusTab4DG(String TAGID)
    {
        JArray ja = new JArray();
        JObject jo = new JObject();
        var query = from t in db.RMAST where t.JOBTAG == TAGID && t.RSTA == "" orderby t.TAGID ascending select t;
        foreach (var item in query)
        {
            var itemObject = new JObject
                    {                                           
    ...
    public JArray getASST(String TAGID)
    {
        JObject jo = new JObject();
        JArray ja = new JArray();
