...
W_SQL = "SELECT FEEYM,OTHERN,PAYAMT,LOGDT,RMNO,MAN,FEETP,FEEAMT,NOTES,LOGUSR, " + ;
    ...
    public JObject getDG3(String TAGID)
    {
        ...
        var query = (from t in db.OTHERFEE where t.RMTAG == TAGID && t.AMTSUM != t.PAYAMT && t.RSTA == " " select new { CNO = "2", TAGID = t.TAGID, FEETP = t.FEETP, FEEYM = t.FEEYM, FEENM = t.OTHERN, FEEAMT = t.AMTSUM, PAYAMT = t.PAYAMT, LOGUSR = t.LOGUSR,YETAMT=t.AMTSUM - t.PAYAMT }).Union
                  (from t in db.RMFEEM
                   where t.RMTAG == TAGID && t.AMTSUM != t.PAYAMT && t.RSTA == " "
                   join b in db.CONFCODE on t.FEETP equals b.CODE_NO into b1
                   from bb in b1
                   where bb.CODE_KIND == "WCOD"
                   select new { CNO = "1", TAGID = t.TAGID, FEETP = t.FEETP, FEEYM = t.FEEYM, FEENM = bb.CODE_NAME, FEEAMT = t.AMTSUM, PAYAMT = t.PAYAMT, LOGUSR = "",YETAMT=t.AMTSUM - t.PAYAMT }).Union
                  (from t in db.CLUBFEE where t.RMTAG == TAGID && t.AMT != t.PAYAMT && t.RSTA == " " select new { CNO = "3", TAGID = t.TAGID, FEETP = "91", FEEYM = t.FEEYM, FEENM = t.ITM, FEEAMT = t.AMT, PAYAMT = (int)t.PAYAMT, LOGUSR = t.LOGUSR, YETAMT = t.AMT - (int)t.PAYAMT });
    ...
    public int doSave(Frm209Poco param)
    {
        ...
        try
        {
            using (db)
            {
                String tagid = StringUtils.getString(param.TAGID);
                ...
                obj.FEETAG = StringUtils.getString(param.TAGID).Trim();
                ...
                if (StringUtils.getString(param.CNO).Trim() == "1"){
                    db.Database.ExecuteSqlCommand("update RMFEEM set PAYAMT={0} where TAGID={1}", param.PAYAMT, tagid);
                }
                else if (StringUtils.getString(param.CNO).Trim() == "2")
                {
                    db.Database.ExecuteSqlCommand("update OTHERFEE set PAYAMT={0} where TAGID={1}", param.PAYAMT, tagid);
                }
                else if (StringUtils.getString(param.CNO).Trim() == "3")
                {
                    db.Database.ExecuteSqlCommand("update CLUBFEE set PAYAMT={0} where TAGID={1}", param.PAYAMT, tagid);
                }
    ...
    public int doSave2(Frm209Poco param,List<Frm209aPoco> LIST)
    {
        ...
        try
        {
            using (db)
            {
                ...
                foreach (Frm209aPoco o in LIST)
                {
                    String tagid = StringUtils.getString(o.TAGID);
                    PAYFEE obj = new PAYFEE();
                    obj.FEEAMT = Convert.ToInt32(o.FEEAMT);
                    ...
                    obj.RMNO = StringUtils.getString(param.RMNO).Trim();
                    /*
                    if (obj.FEEAMT != Convert.ToInt32(o.YETAMT))
                    {
                        obj.PAYAMT = obj.FEEAMT -  Convert.ToInt32(o.YETAMT);  
                    }
                    else
                    {
                        obj.PAYAMT = Convert.ToInt32(o.YETAMT);
                    }*/
                    obj.PAYAMT = Convert.ToInt32(o.YETAMT);
                    ...
                    if (StringUtils.getString(o.CNO).Trim() == "1")
                    {
                        db.Database.ExecuteSqlCommand("update RMFEEM set PAYAMT={0} where TAGID={1}", o.FEEAMT, tagid);
                    }
                    else if (StringUtils.getString(o.CNO).Trim() == "2")
                    {
                        db.Database.ExecuteSqlCommand("update OTHERFEE set PAYAMT={0} where TAGID={1}", o.FEEAMT, tagid);
                    }
                    else if (StringUtils.getString(o.CNO).Trim() == "3")
                    {
                        db.Database.ExecuteSqlCommand("update CLUBFEE set PAYAMT={0} where TAGID={1}", o.FEEAMT, tagid);
                    }
    ...
    public int doAdd(Frm209Poco param)
    {
    ...
    public JObject getHouseGrid(int page = 1, int pageSize = 20, String propertyName = "", String order = "asc", String q = "")
    {
        ...
        "WHERE CNSTA = 'C' 
        */
        var query = from t in db.CONTRAH where t.CNSTA == " " select t;
        if (q != "")
        {
