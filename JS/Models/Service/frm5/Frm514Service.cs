...
public class Frm514Service 
{
    private RENTEntities db = new RENTEntities();
    private CommonService cs = new CommonService();


    public DataTable getReport(Frm514Poco obj)
    {
        ...
        String sql = @"select CAST(CAST(substring(h.rmno,1,2) as decimal) as varchar) + 'F - 1' +substring(h.rmno,3,2)  as RMTXT,
                    ...
                    and t.RSTA =' '";
        */
        var query = from t in db.PARKFEE where t.RSTA==" " && t.FEEYM==obj.FEEYM orderby t.FEEDT select t;


     //   var query = db.Database.SqlQuery<Frm5142Poco>(sql);
