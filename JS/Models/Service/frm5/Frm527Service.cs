...
public class Frm527Service 
{
    private RENTEntities db = new RENTEntities();
    private CommonService cs = new CommonService();


    public DataTable getReport(Frm527Poco obj)
    {

        LinqExtensions le = new LinqExtensions();
        /*
W_SQL = "SELECT * FROM CLASBOOK " + ;
        ...
        */
        ...
        var query = from t in db.CLASBOOK where t.RSTA == " " && t.DT >= dt1 && t.DT <= dt2 select t;

        if (StringUtils.getString(obj.SORTYPE) != "")
        {
            if(obj.SORTYPE=="1"){
                query = query.OrderBy(t => t.DT).OrderBy(t=>t.CLASNM).OrderBy(t=>t.RMNO);
            }else{
                query = query.OrderBy(t => t.CLASNM).OrderBy(t=>t.RMNO);
            }
        }

     //   var query = db.Database.SqlQuery<Frm5272Poco>(sql);
